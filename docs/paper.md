...
# Two Independent Residues in Schema Compilation: A Categorical Framework
...
- **Schema-level residue (conjectural, M2).** A schema residue $\eta^{\mathrm{sch}} : \mathrm{id}_{\mathcal{L}_1} \Rightarrow G \circ \Delta$ requires a right adjoint $G : \mathcal{L}_2 \to \mathcal{L}_1$, which exists if and only if $\mathrm{Hom}_{\mathcal{L}_2}(\Delta(-), b)$ is representable for every $b \in \mathcal{L}_2$. This is conjecture M2; it is open.
...
**Conjecture 4.8 (M2 — Schema-level adjunction via representability).** For every $b \in \mathcal{L}_2$, the presheaf $P_b = \mathrm{Hom}_{\mathcal{L}_2}(\Delta(-), b)$ is representable. That is, there exists an object $G(b) \in \mathcal{L}_1$ and a natural isomorphism
