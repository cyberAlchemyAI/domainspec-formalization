...
- **Open problem identified, and two resolved.** M2-restricted (schema-level representability under a yet-to-be-identified restriction on $\Delta$ — the unrestricted form is refuted in `M2Counter.lean`) remains the one genuinely open problem. M6' (faithfulness implies $\eta^{\mathrm{ins}}_I$ pointwise monic for all $I$) is now settled: *refuted* for universal $I$ (`Bicyclic.lean`, `lanUnit_app_not_mono_bicyclic`) and *proven* on the ind-fragment $\mathrm{Ind}(\mathcal{L}_1)$ (`M6PrimeOnInd.lean`, `unit_mono_on_Ind`). The M6-restricted variant — coherence on a reflective subcategory of $\mathbf{Set}^{\mathcal{L}_1}$ — is *proven* (`M6Restricted.lean`, `m6_restricted`): the fragment is exactly the essential image of $\Delta^*$.
...
The real difficulty lies in the lift from representables to arbitrary $I$. Every $I \in \mathbf{Set}^{\mathcal{L}_1}$ is a colimit of representables, $\Sigma_\Delta = \mathrm{Lan}_\Delta$ preserves colimits as a left adjoint, and $\Delta^*$ preserves both limits and colimits, so $\eta^{\mathrm{ins}}_I$ is the colimit of the representable units $\eta^{\mathrm{ins}}_{\mathrm{y}(c_i)}$ in $\mathbf{Set}^{\mathcal{L}_1}$. Pointwise monomorphisms in $\mathbf{Set}$ are preserved by *filtered* colimits, but not by arbitrary ones: a coproduct of two monos can fail to be mono if the components share targets, and pushouts can collapse distinct preimages. Whether faithfulness of $\Delta$ alone suffices to keep the lifted unit pointwise monic is exactly what M6′ asked — and it is now settled by the colimit shape: on **filtered** colimits (the ind-fragment $\mathrm{Ind}(\mathcal{L}_1)$) faithfulness suffices and the unit stays monic (`M6PrimeOnInd.lean`, `unit_mono_on_Ind`); for non-filtered colimits it can fail, witnessed by a bicyclic torsion act (`Bicyclic.lean`, `lanUnit_app_not_mono_bicyclic`). So Conjecture 7.2 holds on $\mathrm{Ind}(\mathcal{L}_1)$ and is false in general.
...
One problem remains open; two have since been settled. M2 (still open) asks whether $\mathrm{Hom}_{\mathcal{L}_2}(\Delta(-), b)$ is representable for every $b \in \mathcal{L}_2$ — the condition under which the schema-level right adjoint $G$ and the unit $\eta^{\mathrm{sch}}$ exist. M6' asked whether faithfulness of $\Delta$ alone implies that $\eta^{\mathrm{ins}}_I$ is pointwise monic for all $I$; the answer is *no* in general (`Bicyclic.lean`) but *yes* on the ind-fragment $\mathrm{Ind}(\mathcal{L}_1)$ (`M6PrimeOnInd.lean`) — faithfulness suffices exactly when the colimit presentation of $I$ is filtered. M6-restricted asked for a nontrivial reflective subcategory on which injectivity and faithfulness force $\eta^{\mathrm{ins}}_I$ to be iso; it is *proven* (`M6Restricted.lean`), the fragment being the essential image of $\Delta^*$.
...
A coreflective is the limiting case where both residues vanish — schema round-trip exact, instance round-trip exact, $\eta^{\mathrm{sch}}$ and $\eta^{\mathrm{ins}}$ both natural isomorphisms (Definition 5.8). Everything else has positive residue on at least one of the two layers, and which one matters depends on what is being audited. M2 marks the live frontier of the framework — the point at which formal reasoning hands off to either further proof or calibrated empirical practice — while M6′ and M6-restricted have since been settled in Lean (refuted-in-general / proven-on-Ind, and proven, respectively).
...
- [DomainSpec.lean](../lean-formalization/DomainSpec.lean) — The full two-layer residue framework, including the open M2 conjecture and the (now-settled) M6′ and M6-restricted statements (§7).
